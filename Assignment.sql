...
CREATE TABLE DESIGNATION_MASTERS (Design_Code INT PRIMARY KEY,Design_Name VARCHAR(50) UNIQUE);
...
CREATE TABLE DEPARTMENT_MASTERS ( Dept_code INT PRIMARY KEY, Dept_Name VARCHAR(50) UNIQUE);
...
CREATE TABLE STUDENT_MASTERS (Student_Code INT PRIMARY KEY,Student_Name VARCHAR(50) NOT NULL,Dept_Code INT REFERENCES DEPARTMENT_MASTERS(dept_code),Student_Dob DATE,Student_Address VARCHAR(240)
);
...
SELECT Staff_Name AS Name, Staff_Sal AS Salary, Dept_Code FROM STAFF_MASTERS WHERE Dept_Code IN (20, 30, 40);
...
SELECT Student_Code AS Code, Subject1 + Subject2 + Subject3 AS Total_Marks FROM STUDENT_MARKS;
...
SELECT Staff_Name AS Name, Design_Code AS Designation_Code FROM STAFF_MASTERS WHERE Hiredate < '2003-01-01' AND Staff_Sal BETWEEN 12000 AND 25000;
